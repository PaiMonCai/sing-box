...
func (l *Listener) ListenTCP() (net.Listener, error) {
	...
	wrappedListener, err := wrapProxyProtocolListener(tcpListener, l.listenOptions)
	if err != nil {
		_ = tcpListener.Close()
		return nil, err
	}
	tcpListener = wrappedListener
	if l.listenOptions.ProxyProtocol || l.listenOptions.ProxyProtocolAcceptNoHeader {
		l.logger.Info("proxy protocol enabled with trusted upstreams")
	}
